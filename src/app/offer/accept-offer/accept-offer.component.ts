...
export class AcceptOfferComponent implements OnInit {
  ...
  async agree(flag:string){
    this.result = await this.offerService.acceptOffer(this._id, flag);
    if (this.result == undefined || this.result ==null ) {
      this._snackBar.open('DB is not connected. Please try it again.', 'Close');       
    } else {
      (flag == "1")? this._snackBar.open('Successfully accepted.', 'Close') :  this._snackBar.open('Successfully declined.', 'Close');  
